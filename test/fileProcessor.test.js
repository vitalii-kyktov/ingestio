...
describe('fileProcessor.js', () => {
  ...
  describe('File Relationship System', () => {
    it('should group related files by basename', async () => {
      ...
      const videoWithSrtGroup = fileGroups.find(group => group.companionFiles.some(file => file.endsWith('.SRT')))
      ...
      const standaloneVideoGroup = fileGroups.find(group => group.primaryFile.endsWith('0020.MP4'))
      ...
      const imageGroup = fileGroups.find(group => group.primaryFile.endsWith('standalone.JPG'))
    ...
    it('should process file groups with consistent naming', async () => {
      ...
      const group = {
        files: [join(testDir, 'DJI_20250706141254_0019.MP4'), join(testDir, 'DJI_20250706141254_0019.SRT')],
